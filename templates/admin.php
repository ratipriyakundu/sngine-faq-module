    ...
    switch ($_GET['view']) {
        ...
        case 'faq':
            ...
            switch ($_GET['sub_view']) {
                case '':
                    // page header
                    page_header($control_panel['title'] . " &rsaquo; " . __("FAQ Pages"));

                    // get data
                    $get_rows = $db->query("SELECT * FROM system_languages") or _error('SQL_ERROR');
                    if ($get_rows->num_rows > 0) {
                        while ($row = $get_rows->fetch_assoc()) {
                            $faq_created = $db->query("SELECT * FROM faqs WHERE language_id = " . $row['language_id'] . " LIMIT 1") or _error('SQL_ERROR');
                            $row['faq_exists'] = false;
                            $row['faq_enabled'] = false;
                ...
                case 'add':
                    ...
                    // page header
                    page_header($control_panel['title'] . " &rsaquo; " . __("FAQ Pages") . " &rsaquo; " . __("Add New"));
                    break;
